(() => {
  ...
  style.textContent = `
    .motivation-bar{
      ...
      color:#fff;padding:11px 14px;margin:0 0 14px;display:flex;align-items:center;
    ...
    .motivation-bar .motivation-bar-sub{color:#94a3b8;font-size:10px;letter-spacing:.08em;font-weight:800;margin-top:3px}
    .motivation-bar .motivation-chevron{font-size:20px;color:#c4b5fd}
    #motivation{position:relative;min-height:calc(100vh - 130px);overflow:hidden;border-radius:28px}
    #motivation::before{content:"";position:absolute;inset:0;background:url('/images/motivation-bg.webp?v=43') center 18%/cover no-repeat;opacity:.30;pointer-events:none}
    #motivation::after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.62),rgba(2,6,23,.88) 55%,rgba(2,6,23,.95));pointer-events:none}
    ...
    .motivation-thumb{position:relative;aspect-ratio:1/1;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,.14);background:#020617;box-shadow:0 12px 34px rgba(0,0,0,.38)}
    ...
    .motivation-video-title{font-size:13px;font-weight:900;margin:7px 2px 0;line-height:1.2}
    @media(max-width:350px){.motivation-grid{grid-template-columns:1fr}}
  `;
  document.head.appendChild(style);

  const home = document.getElementById("home");
  if (!home || document.getElementById("motivation")) return;
      ...
      <div class="motivation-grid">
        ${videos.map((v, i) => `
          <a class="motivation-video" href="https://youtu.be/${v.id}" target="_blank" rel="noopener noreferrer" aria-label="Open ${v.title} on YouTube">
            ...
            <div class="motivation-video-title">${v.title}</div>
          </a>`).join("")}
      </div>
    </div>`;
  ...
  function closeMotivation(){ showTab("home"); }

  bar.addEventListener("click", openMotivation);
  section.querySelector(".motivation-back").addEventListener("click", closeMotivation);
})();
